...
The latest 2026-09-21 rendered real-Switch run progresses beyond merged #209's PAL `GXSetTevOp (0x80171C4C)` bridge and exposes the distinct next DIRECT blocker `GXSetTevOrder (0x8017214C)` with captured `r3 = 0`. Pinned WiiCompiled `a135beb201042b20f390c6695ca6b26768820fb4` consumes `r3-r6` as TEV stage / texcoord / texmap / channel, rejects stages outside `GX_MAX_TEVSTAGE`, then calls Aurora `GXSetTevOrder`. The blocker diagnostic does not record `r4-r6`, so no hardware texcoord, texmap, or channel values are invented; the bridge reads the live guest registers. The final blocker records stage `RMCP01_GX_SET_TEV_OP`, proving progression past the previous bridge. FIFO traffic remains at eleven writes, while the renderer/FST path remains valid and there is still no proven display list, drawable FIFO work, `GXCopyDisp`, or present. The next patch is restricted to this exact `GXSetTevOrder` boundary.
