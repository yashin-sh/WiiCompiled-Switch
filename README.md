...
The 2026-09-21 rendered real-Switch run progresses beyond merged #204's PAL `GXSetChanCtrl (0x80170570)` bridge and exposes the distinct next DIRECT blocker `GXSetNumTexGens (0x8016E5A4)` with captured `r3 = 0`. Pinned WiiCompiled `a135beb201042b20f390c6695ca6b26768820fb4` implements that boundary as `GXSetNumTexGens((u8)r3)`. The earlier periodic durable snapshot predates the final transition, but the final blocker records stage `RMCP01_GX_SET_CHAN_CTRL`, set inside the previous bridge, before reaching the new target. The renderer/FST path remains valid; the pre-blocker snapshot still records nine FIFO writes with no display list, drawable FIFO work, `GXCopyDisp`, or present. The next patch is restricted to this exact `GXSetNumTexGens` boundary.
