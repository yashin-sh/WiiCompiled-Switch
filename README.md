...
The latest 2026-09-21 rendered real-Switch run progresses beyond merged #207's PAL `GXSetNumIndStages (0x80171B38)` bridge and exposes the distinct next DIRECT blocker `GXSetNumTevStages (0x801722A8)` with captured `r3 = 1`. Pinned WiiCompiled `a135beb201042b20f390c6695ca6b26768820fb4` rejects counts above `GX_MAX_TEVSTAGE`, then calls `GXSetNumTevStages((u8)r3)`. The final blocker records stage `RMCP01_GX_SET_NUM_IND_STAGES`, proving progression past the previous bridge. FIFO traffic has advanced from nine to eleven writes, while the renderer/FST path remains valid and there is still no proven display list, drawable FIFO work, `GXCopyDisp`, or present. The next patch is restricted to this exact `GXSetNumTevStages` boundary.
