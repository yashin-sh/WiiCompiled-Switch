...
The latest 2026-09-21 rendered real-Switch run progresses beyond merged #210's PAL `GXSetTevOrder (0x8017214C)` bridge and exposes the distinct next DIRECT blocker `GXSetBlendMode (0x8017277C)` with captured `r3 = 0`. Pinned WiiCompiled `a135beb201042b20f390c6695ca6b26768820fb4` consumes `r3-r6` as blend mode / source factor / destination factor / logic op and forwards their enum casts directly to Aurora `GXSetBlendMode`. The blocker diagnostic does not record `r4-r6`, so no hardware factor or logic-op values are invented; the bridge reads the live guest registers. The final blocker records stage `RMCP01_GX_SET_TEV_ORDER`, proving progression past the previous bridge. FIFO traffic remains at eleven writes, while the renderer/FST path remains valid and there is still no proven display list, drawable FIFO work, `GXCopyDisp`, or present. The next patch is restricted to this exact `GXSetBlendMode` boundary.
