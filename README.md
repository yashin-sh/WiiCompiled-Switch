...
The latest 2026-09-21 rendered real-Switch run progresses beyond merged #208's PAL `GXSetNumTevStages (0x801722A8)` bridge and exposes the distinct next DIRECT blocker `GXSetTevOp (0x80171C4C)` with captured `r3 = 0`. Pinned WiiCompiled `a135beb201042b20f390c6695ca6b26768820fb4` consumes `r3/r4` as TEV stage/mode, rejects stages outside `GX_MAX_TEVSTAGE`, then calls Aurora `GXSetTevOp`. The blocker diagnostic does not record `r4`, so no hardware mode value is invented; the bridge reads the live guest register. The final blocker records stage `RMCP01_GX_SET_NUM_TEV_STAGES`, proving progression past the previous bridge. FIFO traffic remains at eleven writes, while the renderer/FST path remains valid and there is still no proven display list, drawable FIFO work, `GXCopyDisp`, or present. The next patch is restricted to this exact `GXSetTevOp` boundary.
