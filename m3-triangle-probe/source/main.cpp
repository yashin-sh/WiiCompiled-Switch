...
constexpr const char* kReportPath =
    "sdmc:/switch/WiiCompiled-Switch/m3-vulkan-triangle-probe.txt";
...
struct Probe {
    ...
    bool init_instance() {
        ...
        const VkApplicationInfo app_info{
            ...
            .pNext = nullptr,
            .pApplicationName = "WiiCompiled-Switch M3 Vulkan Triangle Probe",
    ...
    bool init_device() {
        ...
        d.create_image_view = load_device<PFN_vkCreateImageView>("vkCreateImageView");
        d.destroy_image_view = load_device<PFN_vkDestroyImageView>("vkDestroyImageView");
        d.create_render_pass = load_device<PFN_vkCreateRenderPass>("vkCreateRenderPass");
        d.destroy_render_pass = load_device<PFN_vkDestroyRenderPass>("vkDestroyRenderPass");
        d.create_framebuffer = load_device<PFN_vkCreateFramebuffer>("vkCreateFramebuffer");
        d.destroy_framebuffer = load_device<PFN_vkDestroyFramebuffer>("vkDestroyFramebuffer");
        d.create_shader_module = load_device<PFN_vkCreateShaderModule>("vkCreateShaderModule");
        d.destroy_shader_module = load_device<PFN_vkDestroyShaderModule>("vkDestroyShaderModule");
        d.create_pipeline_layout =
            load_device<PFN_vkCreatePipelineLayout>("vkCreatePipelineLayout");
        d.destroy_pipeline_layout =
            load_device<PFN_vkDestroyPipelineLayout>("vkDestroyPipelineLayout");
        d.create_graphics_pipelines =
            load_device<PFN_vkCreateGraphicsPipelines>("vkCreateGraphicsPipelines");
        d.destroy_pipeline = load_device<PFN_vkDestroyPipeline>("vkDestroyPipeline");
        d.cmd_begin_render_pass =
            load_device<PFN_vkCmdBeginRenderPass>("vkCmdBeginRenderPass");
        d.cmd_bind_pipeline = load_device<PFN_vkCmdBindPipeline>("vkCmdBindPipeline");
        d.cmd_draw = load_device<PFN_vkCmdDraw>("vkCmdDraw");
        d.cmd_end_render_pass =
            load_device<PFN_vkCmdEndRenderPass>("vkCmdEndRenderPass");
        ...
        const bool complete =
            ...
            d.begin_command_buffer && d.end_command_buffer &&
            d.create_image_view && d.destroy_image_view &&
            d.create_render_pass && d.destroy_render_pass &&
            d.create_framebuffer && d.destroy_framebuffer &&
            d.create_shader_module && d.destroy_shader_module &&
            d.create_pipeline_layout && d.destroy_pipeline_layout &&
            d.create_graphics_pipelines && d.destroy_pipeline &&
            d.cmd_begin_render_pass && d.cmd_bind_pipeline && d.cmd_draw &&
            d.cmd_end_render_pass && d.create_semaphore &&
            d.destroy_semaphore && d.queue_submit && d.queue_wait_idle &&
            d.device_wait_idle;
        if (!complete) {
    ...
    bool init_swapchain() {
        ...
        if ((capabilities.supportedUsageFlags & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) == 0) {
            report("FAIL: surface images do not support COLOR_ATTACHMENT");
            return false;
        }
    ...
    bool init_graphics() {
        image_views.resize(images.size(), VK_NULL_HANDLE);
        ...
        for (std::size_t i = 0; i < images.size(); ++i) {
            ...
            const VkResult result =
                d.create_image_view(device, &view_info, nullptr, &image_views[i]);
            if (result != VK_SUCCESS) {
                report("FAIL image view index=%zu result=%d", i, result);
                return false;
            }
        }
        ...
        VkResult result =
            d.create_render_pass(device, &render_pass_info, nullptr, &render_pass);
        report("vkCreateRenderPass -> %d", result);
        if (result != VK_SUCCESS) {
            return false;
        }

        for (std::size_t i = 0; i < image_views.size(); ++i) {
            const VkImageView attachment = image_views[i];
            ...
            result =
                d.create_framebuffer(device, &framebuffer_info, nullptr, &framebuffers[i]);
            if (result != VK_SUCCESS) {
                report("FAIL framebuffer index=%zu result=%d", i, result);
                return false;
            }
        }

        auto create_shader = [&](const std::uint32_t* words,
            ...
            const VkResult shader_result =
                d.create_shader_module(device, &shader_info, nullptr, module);
            report("vkCreateShaderModule(%s) -> %d", label, shader_result);
            return shader_result == VK_SUCCESS;
        };

        VkShaderModule vertex_shader = VK_NULL_HANDLE;
        VkShaderModule fragment_shader = VK_NULL_HANDLE;
        if (!create_shader(kTriangleVertSpv,
                           sizeof(kTriangleVertSpv),
                           &vertex_shader,
                           "vert") ||
            !create_shader(kTriangleFragSpv,
                           sizeof(kTriangleFragSpv),
                           &fragment_shader,
                           "frag")) {
            if (vertex_shader != VK_NULL_HANDLE) {
                d.destroy_shader_module(device, vertex_shader, nullptr);
            }
            if (fragment_shader != VK_NULL_HANDLE) {
                d.destroy_shader_module(device, fragment_shader, nullptr);
            }
            return false;
        }
        ...
        result = d.create_pipeline_layout(device, &layout_info, nullptr, &pipeline_layout);
        report("vkCreatePipelineLayout -> %d", result);
        if (result != VK_SUCCESS) {
            d.destroy_shader_module(device, fragment_shader, nullptr);
            d.destroy_shader_module(device, vertex_shader, nullptr);
            return false;
        ...
        result = d.create_graphics_pipelines(
            device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline);
        report("vkCreateGraphicsPipelines -> %d", result);
    ...
    bool record_triangle(std::uint32_t image_index, std::uint64_t frame) {
        ...
        const VkClearValue clear{
            .color = {.float32 = {0.025f, 0.035f, 0.06f, 1.0f}},
        };
        ...
        d.cmd_bind_pipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
        d.cmd_draw(command_buffer, 3, 1, 0, 0);
    ...
    bool run() {
        ...
        report("ENTER TRIANGLE PRESENT LOOP -- press + to exit");
        ...
        while (appletMainLoop()) {
            ...
            if (!record_triangle(image_index, frame)) {
                return false;
            }
            ...
            ++frame;
            if (frame == 1) {
                report("PASS FIRST_TRIANGLE_PRESENT");
    ...
    void shutdown() {
        ...
        if (device != VK_NULL_HANDLE && pipeline != VK_NULL_HANDLE &&
            d.destroy_pipeline) {
            d.destroy_pipeline(device, pipeline, nullptr);
            pipeline = VK_NULL_HANDLE;
        }
        if (device != VK_NULL_HANDLE && pipeline_layout != VK_NULL_HANDLE &&
            d.destroy_pipeline_layout) {
            d.destroy_pipeline_layout(device, pipeline_layout, nullptr);
            pipeline_layout = VK_NULL_HANDLE;
        }
        if (device != VK_NULL_HANDLE && d.destroy_framebuffer) {
            for (VkFramebuffer framebuffer : framebuffers) {
                if (framebuffer != VK_NULL_HANDLE) {
                    d.destroy_framebuffer(device, framebuffer, nullptr);
                }
            }
            framebuffers.clear();
        }
        if (device != VK_NULL_HANDLE && render_pass != VK_NULL_HANDLE &&
            ...
            d.destroy_render_pass(device, render_pass, nullptr);
            render_pass = VK_NULL_HANDLE;
        }
        if (device != VK_NULL_HANDLE && d.destroy_image_view) {
            for (VkImageView image_view : image_views) {
                if (image_view != VK_NULL_HANDLE) {
                    d.destroy_image_view(device, image_view, nullptr);
                }
            }
            image_views.clear();
        }
...
int main(int, char**) {
    ...
    report("goal: VK_NN_vi_surface -> NVK graphics pipeline -> triangle -> present");
    ...
    const bool initialized = probe.init_instance() && probe.init_surface() &&
                             ...
                             probe.init_commands() && probe.init_graphics();
    bool passed = false;
    if (initialized) {
        passed = probe.run();
    } else {
