...
readonly DAWN_BUILD_DIR="${MKW_M3_AURORA_BUILD_ROOT:-$DEPS_DIR/dawn-switch-aurora-gx-build}"
...
readonly OUTPUT="$OUTPUT_DIR/WiiCompiled-Switch-m3-aurora-gx-probe.nro"
...
if [[ ! -f "$PROBE_SOURCE" || ! -f "$PROBE_DIR/CMakeLists.txt" ]]; then
    echo "error: missing Aurora GX probe sources under $PROBE_DIR" >&2
    exit 1
fi
...
if [[ "${MKW_M3_FORCE_AURORA_RECONFIGURE:-0}" == "1" ]]; then
    rm -rf "$DAWN_BUILD_DIR"
    mkdir -p "$DAWN_BUILD_DIR"
fi

echo "[4/7] Configuring pinned Aurora GX + Dawn Switch path against proven NVK..."
docker run --rm \
    "${DOCKER_SECURITY_ARGS[@]}" \
    -e MKW_M3_JOBS="$JOBS" \
...
echo "[5/7] Building Aurora GX triangle probe..."
docker run --rm \
    "${DOCKER_SECURITY_ARGS[@]}" \
    -e MKW_M3_JOBS="$JOBS" \
    -v "$MESA_DIR:/mesa:ro" \
...
if [[ -z "$built_nro" || ! -f "$built_nro" ]]; then
    echo "error: Aurora GX build did not produce dawn_switch_triangle.nro" >&2
    exit 1
fi
cp "$built_nro" "$OUTPUT"
...
echo
echo "Copy it to:"
echo "  /switch/WiiCompiled-Switch-m3-aurora-gx-probe/WiiCompiled-Switch-m3-aurora-gx-probe.nro"
echo
echo "Expected hardware result:"
echo "  - a large RGB triangle appears through real Aurora GX -> Dawn/WebGPU;"
echo "  - press + to exit;"
echo "  - send back /switch/WiiCompiled-Switch/m3-aurora-gx-probe.txt."
