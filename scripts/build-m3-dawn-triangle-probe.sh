...
readonly DAWN_BUILD_DIR="${MKW_M3_DAWN_TRIANGLE_BUILD_ROOT:-$DEPS_DIR/dawn-switch-triangle-build}"
...
readonly PROBE_SOURCE="$ROOT_DIR/m3-dawn-triangle-probe/source/main.cpp"
readonly OUTPUT_DIR="$ROOT_DIR/m3-dawn-triangle-probe"
readonly OUTPUT="$OUTPUT_DIR/WiiCompiled-Switch-m3-dawn-triangle-probe.nro"
...
# The fork's CMake recipe names src/dawn/switch/triangle_nro.cpp while the public
...
echo "[5/7] Building Dawn/WebGPU WGSL triangle probe..."
docker run --rm \
    "${DOCKER_SECURITY_ARGS[@]}" \
    -e MKW_M3_JOBS="$JOBS" \
    -v "$MESA_DIR:/mesa:ro" \
...
if [[ -z "$built_nro" || ! -f "$built_nro" ]]; then
    echo "error: Dawn build did not produce dawn_switch_triangle.nro" >&2
    exit 1
fi
cp "$built_nro" "$OUTPUT"
...
echo
echo "Copy it to:"
echo "  /switch/WiiCompiled-Switch-m3-dawn-triangle-probe/WiiCompiled-Switch-m3-dawn-triangle-probe.nro"
echo
echo "Expected hardware result:"
echo "  - a large RGB triangle appears through Dawn/WebGPU/WGSL;"
echo "  - press + to exit;"
echo "  - send back /switch/WiiCompiled-Switch/m3-dawn-triangle-probe.txt."
