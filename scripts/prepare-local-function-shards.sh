...
dotnet "$CLI_DLL" translate-recursive "$ENTRY" --project "$MANIFEST"

echo "[4/4] Emitting stable translated build shards..."
dotnet "$CLI_DLL" emit-build-shards --project "$MANIFEST"
...
This checkpoint only generates the translated function graph. It does NOT execute
constructors or the Mario Kart Wii entry point. The Switch link step is intentionally
kept separate so compiler/linker dependencies can be attributed cleanly.
EOF
